...
class CommentController extends AppController
{
    ...
    public function liked()
    {
        ...
        if (!$like_checker) { //comment has not been liked
            if (!$dislike_checker) { //comment has not been disliked
                $comment->addLike($comment_id); //add 1 liked record in like_monitor table
                $comment->updateLikedCount($comment_id); //increment liked column in comment table
                ...
                $comment->addLike($comment_id); //add 1 like record in like_monitor table
                $comment->updateLikedCount($comment_id); //increment liked column in comment table
            }
        }
        $this->set(get_defined_vars());
    ...
    public function disliked()
    {
        ...
        if (!$dislike_checker) { //comment has not been disliked
            if (!$like_checker) { //comment has not been liked
                $comment->addDislike($comment_id); //add 1 disliked record in like_monitor table
                $comment->updateDislikedCount($comment_id); //increment disliked column in comment table
                ...
                $comment->addDislike($comment_id); //add 1 dislike record in like_monitor table
                $comment->updateDislikedCount($comment_id); //increment disliked column in comment table
            }
        }
